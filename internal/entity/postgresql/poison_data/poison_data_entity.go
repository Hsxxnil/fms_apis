...
import (
	model "fms/internal/entity/postgresql/db/poison_data"
...
func (s *storage) GetBySIDList(input *model.Base) (output []*model.Table, err error) {
	subQuery := s.db.Model(&model.Table{}).Select("date_time").Order("date_time desc").Limit(10)
	...
	query.Where(filter)
	err = query.Order("date_time asc").Find(&output).Error
	if err != nil {
		log.Error(err)
		return nil, err
	}
...
func (s *storage) GetByLastData(input *model.Base) (output []*model.Table, err error) {
	...
	err = query.Order("date_time desc").Where("date_time >= (? as sub)", subQuery1).Find(&output).Error
	if err != nil {
		log.Error(err)
		return nil, err
	}
