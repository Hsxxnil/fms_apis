...
import (
	...
	model "fms/internal/entity/postgresql/db/transport_order_details"
...
func (s *storage) GetByListNoPagination(input *model.Base) (output []*model.Table, err error) {
	...
	if input.TransportOrderID != nil {
		query.Where("transport_order_id = ?", input.TransportOrderID)
	}
...
func (s *storage) Update(input *model.Base) (err error) {
	...
	if input.ProductName != nil {
		data["product_name"] = input.ProductName
	}
	...
	if input.Tonnage != nil {
		data["tonnage"] = input.Tonnage
	}

	if input.TrailerID != nil {
		data["trailer_id"] = input.TrailerID
	}
